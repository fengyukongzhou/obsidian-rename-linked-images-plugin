...
一个用于批量重命名当前笔记中所有图片链接的 Obsidian 插件，支持统一的命名规则和链接格式转换。
...
- 📋 **顺序保持**：按照图片在文档中出现的顺序进行重命名
- 🔄 **双向转换**：Wiki链接和Markdown链接之间的相互转换
...
1. 打开包含图片链接的笔记
2. 使用命令面板搜索以下命令之一：
   - "Wiki转Markdown链接" - 转换为 `![](图片)` 格式
   - "Markdown转Wiki链接" - 转换为 `![[图片]]` 格式
3. 确认转换操作
...
**有描述的情况**：
- `![[图片|描述]]` ⇄ `![描述](图片)`
- 只有当原本有描述时，才在转换中保留描述信息

### 转换示例

| Wiki格式 | Markdown格式 | 说明 |
|---------|-------------|------|
| `![[zd1130-001.png]]` | `![](zd1130-001.png)` | 无描述，保持简洁 |
| `![[zd1130-001.png|美丽的图片]]` | `![美丽的图片](zd1130-001.png)` | 有描述，相互转换 |
...
2. **顺序保持**：按照图片在文档中出现的顺序提取，避免排序混乱
3. **命名映射生成**：根据配置规则生成旧文件名到新文件名的映射关系
4. **文件重命名**：使用 Obsidian API 重命名实际文件
5. **内容更新**：采用单次遍历替换策略更新笔记中的链接引用
6. **格式转换**：支持Wiki和Markdown链接之间的双向转换

### 关键特性
- **安全性**：检查文件存在性，避免覆盖已有文件
- **容错性**：处理各种异常情况，提供友好提示
- **性能优化**：单次遍历替换，避免重复操作导致的污染问题
- **顺序保证**：严格按照文档中出现顺序进行重命名
- **描述智能处理**：默认无描述，有描述时才保留转换
...
### v1.0.3 (2025-11-30)
- ✅ **添加Markdown转Wiki链接功能**：实现双向链接格式转换
- ✅ **优化链接转换逻辑**：默认无描述，有描述时才保留转换
- 🔧 **改进用户体验**：简化转换流程，提高转换准确性

### v1.0.2 (2025-11-30)
- ✅ **修复图片顺序问题**：现在按照图片在文档中出现的顺序进行重命名
- ✅ **优化链接格式转换**：添加一键切换链接格式功能
- 🔧 **改进提取算法**：使用更精确的正则表达式和顺序保持机制
